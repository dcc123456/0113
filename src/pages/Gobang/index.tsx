...
const Gobang: React.FC = () => {
  ...
  const [board, setBoard] = useState<Board>(() => {
    const initialBoard: Board = Array(BOARD_SIZE).fill(null).map(() => 
  ...
  /** 当前玩家 */
  const [currentPlayer, setCurrentPlayer] = useState<typeof BLACK | typeof WHITE>(BLACK);
   ...
   * @returns 是否获胜
   */
  const checkWin = (board: Board, x: number, y: number, player: Player): boolean => {
    // 检查四个方向的连续棋子
   ...
   * @returns 位置价值分数
   */
  const evaluatePosition = (board: Board, x: number, y: number, player: Player): number => {
    // 评估棋局 根据棋子形态得到value
    const valueMap = {
      11: 1,     // 一边被拦截的单子
      12: 2,     // 两边均不被拦截的单子
      ...
      <div className="w-full max-w-4xl mx-auto bg-white rounded-lg shadow-xl p-6">
        ...
        {gameStatus === 'init' && (
          <div className="flex flex-col items-center justify-center py-8">
            ...
            >
              <h2 className="text-2xl font-bold text-center text-gray-800 mb-8">游戏设置</h2>
              
              {/* 设置内容 */}
              <div className="flex gap-8">
                {/* AI难度设置 */}
                <div className="text-left">
                  <h3 className="text-lg font-semibold text-amber-900 mb-4">AI难度</h3>
                ...
                {/* 先手/后手设置 */}
                <div className="text-left">
                  <h3 className="text-lg font-semibold text-amber-900 mb-4">先手/后手</h3>
                  <div className="flex flex-col gap-3">
                    <label className="flex items-center gap-3 cursor-pointer text-amber-900">
                      <input
                        type="radio"
                        ...
                        value={0}
                        checked={offense === 0}
                        onChange={(e) => setOffense(Number(e.target.value) as Offense)}
                        className="w-4 h-4 text-amber-600 focus:ring-amber-500 border-gray-300"
                      />
                      <span>先手</span>
                    </label>
          ...
          <>
            ...
            <div className="mb-8">
              ...
              <div className="flex items-center justify-center gap-8 mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-xl font-semibold ${currentPlayer === BLACK ? 'text-blue-600' : 'text-gray-500'}`}>
                    黑方
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-xl font-semibold ${currentPlayer === WHITE ? 'text-blue-600' : 'text-gray-500'}`}>
                    白方
                  </span>
                </div>
              </div>
            ...
            <div className="flex justify-center items-center mb-8 w-full">
              <div className="relative">
                {/* 棋盘背景 */}
                <div
                  className="w-[600px] h-[600px] border-8 border-amber-900 rounded-lg shadow-2xl overflow-hidden relative mx-auto"
                  style={{
